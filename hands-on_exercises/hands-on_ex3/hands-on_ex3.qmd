...
First, we will extract the clustering variables from shan_sf simple features object into data.frame. Note that we exclude INTERNET_PR because it is highly correlated with COMPUTER_PR.
...
The code chunk below will be used to compute the agglomerative coefficients of all hierarchical clustering algorithms. We can see that Ward's method provides the strongest clustering structure among the four methods assessed. Hence, in the subsequent analysis, only Ward's method will be used.
...
```{r}
clus6 <- skater(edges=shan.mst[,1:2],
                data=shan_ict,
                method="euclidean",
                ncuts=5)
```
