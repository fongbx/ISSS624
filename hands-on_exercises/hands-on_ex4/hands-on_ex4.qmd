...
1.  [Calibrating Hedonic Pricing Model for Private Highrise Property with GWR Method](https://r4gdsa.netlify.app/chap06.html#building-hedonic-pricing-models-using-gwmodel)

## Getting Started

Install and load the necessary packages.

-   R package for building OLS and performing diagnostics (assumption) tests
...
The report shows that the R object used to contain the imported MP14_SUBZONE_WEB_PL shapefile is called *mpsz* and it is a simple feature object. The geometry type is *multipolygon*. It is also important to note that mpsz simple feature object does not have EPSG information.
...
We will update the newly imported *mpsz* with the correct ESPG code (i.e. 3414). Note that SVY21 is Singapore's code terminology of the projected CRS and we want to transform it to the globally-recognised terminology EPSG 3414. Although SVY21 is supposed to be equivalent to EPSG 3414, there are some minor adjustments/differences so we do a transform instead of set crs.
...
Next, you will reveal the extent of *mpsz_svy21* using `st_bbox()` of sf package.
...
We can plot the distribution of *SELLING_PRICE* by using appropriate Exploratory Data Analysis (EDA) as shown in the code chunk below. The figure reveals a right skewed distribution. This means that more condominium units were transacted at relative lower prices.
...
Next, the code chunks below is used to create an interactive point symbol map. Notice that [`tm_dots()`](https://www.rdocumentation.org/packages/tmap/versions/2.2/topics/tm_symbols) is used instead of `tm_bubbles()`. `set.zoom.limits` argument of `tm_view()` sets the minimum and maximum zoom level to 11 and 14 respectively.
...
`lm()` returns an object of class \"lm\" or for multiple responses of class c(\"mlm\", \"lm\"). The functions `summary()` and `anova()` can be used to obtain and print a summary and analysis of variance table of the results. The generic accessor functions coefficients, effects, fitted.values and residuals extract various useful features of the value returned by `lm`.
...
To visualise the best fit curve on a scatterplot, we can incorporate `lm()` as a method function in ggplot\'s geometry as shown in the code chunk below. The figure reveals that there are a few statistical outliers with relatively high selling prices.
...
corrplot(cor(condo_resale[, 5:23]), diag=FALSE, order="AOE",
         tl.pos="td", tl.cex=0.5, type="upper")
```
...
Next, we will convert *condo_resale.res.sf* from simple feature object into a SpatialPointsDataFrame because spdep package can only process sp conformed spatial data objects. The code chunk below will be used to perform the data conversion process.
...
The tmap_mode is set back to "plot" before we continue.

```{r}
tmap_mode("plot")
```

The figure reveals that there is sign of spatial autocorrelation.
...
Similar to the earlier section, we will first use `bw.gwr()` to determine the recommended data point to use. The code chunk used look very similar to the one used to compute the fixed bandwidth except the `adaptive` argument has changed to **TRUE**.
